...
function App() {
  ...
  setShuffling(true);
  setWinners([]);                    // ← NEW: Clear previous winners
  setShufflingWinners([]);           // ← NEW: Reset shuffling display
  
  // ← NEW: Start shuffling animation
  const shuffleInterval = setInterval(() => {
    ...
    setShufflingWinners(randomComments);  // ← NEW: Update display with random participants
  }, 100); // ← NEW: Change every 100ms for fast shuffle effect
  
  // ← NEW: After 3 seconds, pick the real winners
  setTimeout(async () => {
    clearInterval(shuffleInterval);      // ← NEW: Stop the shuffling
    ...
    try {
      ...
      setShufflingWinners([]);           // ← NEW: Clear shuffling display
      ...
      <CardContent>
        <div className="space-y-4">
          {winners.map((winner, index) => (
            ...
            >
              {/* All the winner card content - profile pic, name, comment, etc */}
            </div>
          ))}
      </CardContent>
    </Card>
